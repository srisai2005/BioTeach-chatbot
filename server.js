...
if (!apiKey) {
  console.error("AZURE_OPENAI_KEY is missing. Set it in your environment or .env file.");
  process.exit(1);
}
...
app.post("/api/chat", async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(500).json({ error: "Azure OpenAI request failed.", details: error.message });
  }
});
...
app.listen(PORT, () => {
  console.log(`Maya Biology Coach is running at http://localhost:${PORT}`);
});
